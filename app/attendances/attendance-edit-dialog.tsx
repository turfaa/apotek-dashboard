...
export function AttendanceEditDialog({
    ...
    useEffect(() => {
        getEmployees().then(employees => {
            const employee = employees.find(emp => emp.id === cellData.employeeID)
            if (employee) {
                                    ...
                                    {attendanceTypes.map(type => (
                                        <SelectItem key={type.id} value={type.id.toString()}>
                                            <div className="flex items-center gap-2">
                                                <span>{type.name}</span>
                                                {type.payableType === "working" && (
                                                    <span className="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded">
                                                        Kerja
                                                    </span>
                                                )}
                                                {type.payableType === "benefit" && (
                                                    <span className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded">
                                                        Benefit
                                                    </span>
                                                )}
                                                {type.payableType === "none" && (
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                        ...
                        <div className="col-span-3">
                            ...
                            <p className="text-xs text-muted-foreground mt-1">
                                Masukkan jumlah jam lembur (contoh: 2.5 untuk 2 jam 30 menit)
                            </p>
                        </div>
                    </div>
                </div>
