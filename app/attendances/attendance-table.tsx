...
import { Attendance, AttendanceType, AttendancesAtDate, upsertAttendance, EmployeeAttendanceSummary } from "@/lib/api/attendance"
...
export function AttendanceTable({ 
    ...
    const [selectedCell, setSelectedCell] = useState<AttendanceCellData | null>(null)
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                const attendanceData = await getAttendances(format(currentMonth, "yyyy-MM"))
    ...
    const getAttendance = useCallback((employeeID: number, date: string): Attendance | undefined => {
        const dateAttendances = attendances.find(a => a.date === date)
        return dateAttendances?.attendances.find(a => a.employeeID === employeeID)
    }, [attendances])

    // Get summary for a specific employee
    const getEmployeeSummary = useCallback((employeeID: number): EmployeeAttendanceSummary | undefined => {
        return employeeSummaries.find(summary => summary.employeeID === employeeID)
    }, [employeeSummaries])

    // Handle cell click
    const handleCellClick = useCallback((employeeID: number, date: string): void => {
    ...
    const handleKeyDown = useCallback((event: React.KeyboardEvent, employeeID: number, date: string): void => {
    ...
    const handleAttendanceUpdate = useCallback(async (data: {
        employeeID: number
        date: string
        typeID: number
        ...
        try {
            ...
            // Refresh the data
            const newAttendanceData = await getAttendances(format(currentMonth, "yyyy-MM"), session)
    ...
    if (isLoading) {
        return (
            <div className="text-center py-8">
                ...
                <p className="mt-2 text-muted-foreground">Loading kehadiran...</p>
            </div>
        )
    }
    ...
    if (filteredEmployees.length === 0) {
        return (
            <div className="text-center py-8">
                <p className="text-muted-foreground">Tidak ada karyawan yang ditampilkan dalam kehadiran.</p>
            </div>
        )
    }
                    ...
                    <div className="overflow-x-auto">
                        ...
                        <TableHeader>
                            <TableRow className="bg-muted/50">
                                <TableHead className="w-32 sticky left-0 bg-muted/50 z-10">Tanggal</TableHead>
                                {filteredEmployees.map(employee => (
                                    <TableHead key={employee.id} className="text-center min-w-[120px]">
                                        {employee.name}
                                    </TableHead>
                                ))}
                            </TableRow>
                        ...
                        <TableBody>
                                        ...
                                        <TableRow key={date}>
                                            ...
                                            {filteredEmployees.map(employee => {
                                                const attendance = getAttendance(employee.id, date)
                                                const isMutable = isDateMutable(date)
                                        ...
                                        return (
                                            ...
                                            role={isMutable ? "button" : undefined}
                                            aria-label={isMutable ? `Edit kehadiran ${employee.name} pada ${format(parseISO(date), "dd MMM yyyy", { locale: id })}` : undefined}
                                        >
                                            <Tooltip>
                                                    ...
                                                    <div className="text-center">
                                                        ...
                                                        {attendance && (
                                                            <div className="text-sm opacity-75 mt-1">
                                                                {attendance.type.name}
                                                                {attendance.overtimeHours > 0 && ` • +${attendance.overtimeHours} jam`}
                                                            </div>
                                                        )}
                                                    </div>
                                                </TooltipContent>
                        ...
                        <TableFooter>
                            <TableRow className="bg-muted/30">
                                ...
                                {filteredEmployees.map(employee => {
                                    ...
                                    if (!summary) {
                                        return (
                                            <TableCell key={employee.id} className="text-center text-sm text-muted-foreground">
                                                -
                                            </TableCell>
                                        )
                                    }
                                    ...
                                    return (
                                        <TableCell key={employee.id} className="text-center text-sm">
                                            <div className="space-y-1">
                                                <div className="font-medium">{summary.workingDays} hari kerja</div>
                                                {benefitEntries.length > 0 && (
                                                    <div className="space-y-0.5">
                                                        {benefitEntries.map(([benefit, count]) => (
                                                            <div key={benefit} className="text-xs text-blue-600">
                                                                {count} {benefit}
                                                            </div>
                                                        ))}
                                                    </div>
                                                ...
                                                {summary.overtimeHours > 0 && (
                                                    <div className="text-xs text-orange-600">
                                                        +{summary.overtimeHours} jam lembur
                                                    </div>
                                                )}
                                            </div>
                                        </TableCell>
