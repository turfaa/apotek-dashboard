import { InvoiceCalculator, InvoiceComponent } from "@/lib/api/invoice-calculator"
        ...
        persist(
            (set) => ({
                ...
                addInvoice: () => set(state => {
                    ...
                    return {
                        invoices: [...state.invoices, newInvoice(state.calculator)]
                    }
                }),

                removeInvoice: (index: number) => set(state => ({
                    invoices: [
                ...
                updateInvoice: (index: number, newInvoice: Invoice) => set(state => ({
                    invoices: [
                        ...state.invoices.slice(0, index),
                        newInvoice,
                        ...state.invoices.slice(index + 1)
...
function newInvoice(calculator: InvoiceCalculator): Invoice {
    return { components: calculator.components.map((c) => ({ ...c, amount: 0 })), ignoreMultiplier: false }
}